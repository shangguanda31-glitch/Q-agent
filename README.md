...
│ :5050    │
└──────────┘
  LLM: 8081 (Qwen3.5-9B) · Embed: 8082 (Qwen3.5-0.8B, CPU)
...
```bash
# 1. 运行 NapCatQQ（WebSocket :4447 + HTTP :4444）
# 2. 编译运行
cd qq-assistant
cargo run --release
...
QAgent is a Rust-based intelligent QQ assistant that monitors QQ messages via NapCatQQ, uses a local LLM (Qwen3.5-9B) for analysis, and autonomously calls tools through an agent loop.

### Features

- **🤖 Agent Loop** — Message → LLM decides → Call tools → Feed back → Continue
- **🧠 3-Layer Memory** — Context + SQLite persistence + Read/Write tools
- **🔧 9 Tools** — Notify / Schedule / Notes / Memory / OCR / Code / QQ Read
...
```bash
# 1. Run NapCatQQ (WebSocket :4447 + HTTP :4444)
# 2. Build & run
cd qq-assistant
cargo run --release
