...
impl Config {
    pub fn from_env() -> Self {
        let base_dir = std::env::var("PROJECT_ROOT")
            .unwrap_or_else(|_| "D:/桌面/编程作品/Sandy ONE".to_string());
        let data_dir = std::env::var("DATA_DIR")
            .unwrap_or_else(|_| format!("{}/qq-assistant/data", base_dir));

        Self {
            napcat_ws_url: std::env::var("NAPCAT_WS_URL")
                .unwrap_or_else(|_| "ws://127.0.0.1:4447".to_string()),
            ...
            llm_model: std::env::var("LLM_MODEL")
                .unwrap_or_else(|_| "Qwen3.5-9B-Q4_K_M".to_string()),
            web_port: std::env::var("WEB_PORT")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(5050),
            llama_server_path: std::env::var("LLAMA_SERVER_PATH")
                .unwrap_or_else(|_| "D:/llm/llama-bin/llama-server.exe".to_string()),
            llama_model_path: std::env::var("LLAMA_MODEL_PATH")
                .unwrap_or_else(|_| "D:/llm/models/Qwen3.5-9B.Q4_K_M.gguf".to_string()),
            llama_gpu_layers: std::env::var("LLAMA_GPU_LAYERS")
                .ok()
                .and_then(|v| v.parse().ok())
                .unwrap_or(99),
            ...
            claude_code_enabled: std::env::var("CLAUDE_CODE_ENABLED")
                .ok().map(|v| v != "0" && v != "false").unwrap_or(true),
            claude_code_timeout_secs: std::env::var("CLAUDE_CODE_TIMEOUT")
                .ok().and_then(|v| v.parse().ok()).unwrap_or(120),
            claude_working_dir: std::env::var("CLAUDE_WORKING_DIR")
                .unwrap_or_else(|_| format!("{}/qq-assistant/claude_workspace", base_dir)),
            data_dir,
        }
    }
}
