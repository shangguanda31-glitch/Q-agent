...
impl Config {
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            llm_url: std::env::var("LLM_URL").unwrap_or_else(|_| "http://127.0.0.1:8081".to_string()),
            embed_url: std::env::var("EMBED_URL").unwrap_or_else(|_| "http://127.0.0.1:8082".to_string()),
            llm_model: std::env::var("LLM_MODEL").unwrap_or_else(|_| "qwen3.5-9b".to_string()),
            llm_connect_timeout_secs: std::env::var("LLM_CONNECT_TIMEOUT").ok().and_then(|v| v.parse().ok()).unwrap_or(10),
            llm_read_timeout_secs: std::env::var("LLM_READ_TIMEOUT").ok().and_then(|v| v.parse().ok()).unwrap_or(600),
            web_port: std::env::var("WEB_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(5050),
            llama_server_path: std::env::var("LLAMA_SERVER_PATH").unwrap_or_else(|_| "llama-server".to_string()),
            llama_model_path: std::env::var("LLAMA_MODEL_PATH").unwrap_or_else(|_| "models/qwen3.5-9b-q4_k_m.gguf".to_string()),
            embed_model_path: std::env::var("EMBED_MODEL_PATH").unwrap_or_else(|_| "models/qwen3.5-0.8b-q6_k.gguf".to_string()),
            ...
            claude_working_dir: std::env::var("CLAUDE_WORKING_DIR").unwrap_or_else(|_| "claude_workspace".to_string()),
            data_dir,
        }
    }
}
