...
# 3. 打开面板 http://127.0.0.1:5050
```

## 配置

### .env 文件
复制 `.env.example` 为 `.env`，填入配置。不会被 git 追踪。
...
系统自动启动两个 llama-server 实例：

| 端口 | 模型 | 用途 | 显存 |
|------|------|------|------|
| 8081 | Qwen3.5-9B (Q4_K_M) | Agent 循环、对话、任务处理 | ~6GB |
| 8082 | Qwen3.5-0.8B (Q6_K) | 语义记忆 embedding | ~0.5GB |
